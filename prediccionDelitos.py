...
st.set_page_config(
  page_title="Prediccion delitos Bucaramanga",
...
Se utilizó modelos supervidados de clasificacion y se obtuvo los mejores resultados 
en la predicción del crimen se dieron con RandomForestClassifier-(Bosque aleatorios)


A pesar de que existen limitaciones con la información útil para la predicción, se probaron 4 opciones y el mejor modelo
arrojó  más del 66 % de exactitud (accuracy.).
Concluimos que los modelos de predicción del delito basado en los articulos del código civil([LINK](https://oig.cepal.org/sites/default/files/2000_codigopenal_colombia.pdf))
...
with st.container():
  ...
  with left_column:
    st.subheader("Las librerias usadas para entrenar el demos fueron")
    st.write(
      """
      El objetivo de este trabajo académico es construir una herramienta en código Python para predecir la categoria
      de delito que se comete tienen en cuentas las siguietes caracterísitas:
      'EDAD', 'GENERO', 'RANGO_HORARIO', 'NOM_COM', 'DIA_NOMBRE'. Probamos y entrenamos modelo con Naybe bayes , Source vector machine
      , arbol de deciones y bosque aleatorio, El modelo elegido fue Bosque Aleatorio bajo el criterio de mejor accurasy frente a los datos de prueba.

      La clasificacion de EDAD se tomo de acuerdo al ciclo de vida que proporciona el ministerio de salud.
      ([INFO](https://www.minsalud.gov.co/proteccionsocial/Paginas/cicloVida.aspx))
...
with st.container():
  ...
  st.write("Edad: ", edad, "equIvalente a",edadn )
  st.write("Género : ", genero,"equIvalente a",generon)
  st.write("Mes :", mes,"equIvalente a",mes)
  st.write("Hora", hora,"equIvalente a", horan)
  st.write("Comuna",comuna,"equIvalente a", comunan)
  st.write("dia",dia,"equivalente a",dian) 
